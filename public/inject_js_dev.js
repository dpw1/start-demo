...
window.ezfyEasyUpsellApp = (function () {
  ...
  window.ezfyCartUpsellTotalProducts = null;
...
`);
  ...
  async function getUpsellProducts() {
    return new Promise(async (resolve, reject) => {
      ...
      Ecwid.Cart.get(async (cart) => {
        var productsInCart = cart.items.map((e) => e.product.id);
        ...
        const _allProducts = await getProductsData();

        const allProducts = _allProducts.filter((e) => e.enabled);
        const productsWithRelatedProducts = allProducts.filter(
          (e) =>
        ...
        for (var _each of productsInCart) {
          const each = productsWithRelatedProducts.filter(
            (e) => e.id === _each,
          )[0];
          ...
          const upsell = each.relatedProducts;

          for (var relatedProductID of each.relatedProducts.productIds) {
            const relatedProduct = await getProductData(relatedProductID);
            upsells.push(relatedProduct);
          }

          if (each.relatedProducts.relatedCategory.enabled) {
            categoryProducts = await getProductsFromCategoryWithId(
              each,
              productsInCart,
            );

            if (categoryProducts.length >= 1) {
              upsells.push(...categoryProducts);
            }
          }
        }

        const __filtered = upsells.slice(0, 10);

        var repeated = new Set();

        const _filtered = __filtered.filter(function (obj) {
          if (!repeated.has(obj.id)) {
            repeated.add(obj.id);
            return true;
          }
          return false;
  ...
  function getProductsFromCategoryWithId(product, productsInCart) {
    return new Promise(async (resolve, reject) => {
      let categories = [];

      const relatedProducts = product.relatedProducts;
      const id = relatedProducts.relatedCategory.categoryId;
      const quantity = relatedProducts.relatedCategory.productCount;

      if (window.ezfyCategories) {
        categories = window.ezfyCategories;
      } else {
        var url = `https://app.ecwid.com/api/v3/${window.Ecwid.getOwnerId()}/categories?token=${
      ...
      const __products = await getProductsData();
      const _products = removeUnavailableProducts(__products);
      const products = _products
        .filter((item) => item.id !== product.id)
        .filter((itemA) => !productsInCart.includes(itemA.id));
      ...
      if (products && products.length >= 1) {
        if (id !== 0) {
          if (products && products.length >= 1) {
            found.push(...products.filter((e) => e.categoryIds.includes(id)));
          }
        } else {
          /* get random products */

          const list = products.filter(
            (itemA) => !relatedProducts.productIds.includes(itemA.id),
          );
          ...
          if (list.length >= 1) {
            ...
            found.push(
              ...[...new Set(_found.map((item) => item.id))].map((id) =>
                _found.find((item) => item.id === id),
              ),
            );
          }
        }
  ...
  function removeUnavailableProducts(products) {
    return products
      .filter((e) => {
  ...
  function ensureUpsellProductsAreSyncWithAPI(products) {
    return new Promise(async (resolve) => {
      const _allProducts = await getProductsData();
      const allProducts = removeUnavailableProducts(_allProducts);

      const result = allProducts.filter((obj1) =>
        products.some((obj2) => obj2.id === obj1.id),
      );
  ...
  function getProductsData() {
    return new Promise((resolve, reject) => {
      ...
      fetch(url)
        ...
        .then((products) => {
          window.store__products = products.items;
  ...
  function getProductData(id) {
    return new Promise(async (resolve, reject) => {
      ...
      if (window.hasOwnProperty("store__products")) {
        try {
          const found = window.store__products.filter((e) => e.id === id)[0];
          resolve(found);
          return;
        } catch (err) {}
      }
      ...
      found = products.filter((e) => e.id === id)[0];
      resolve(found);
    });
  }
  ...
  async function injectCartUpsell(_products, restartType = null) {
    return new Promise(async (resolve) => {
      ...
      if ($ezfycart && _products.length <= 0) {
        try {
          $ezfycart.remove();
        } catch (err) {}
      }
      ...
      for (var each of _products) {
        ...
        const isPopup = parseInt(each.options.length) <= 0 ? false : true;

        let button = "";

        if (ezfyUpsellData.settings.enableAddToCart) {
          button += `
        	      <button 
...
<span class="EzfyCart-atc-text">${
      ...
      <div class="EzfyCart-atc-loading">
        ...
        if (
          each.galleryImages.length >= 1 &&
          window.ezfyUpsellData.settings.enableSecondImageOnHover
        ) {
          secondImage += `
        		<img 
        		loading="lazy"
        		class="EzfyCart-image EzfyCart-image-hover" src="${each.galleryImages[0].thumbnailUrl}" alt="${each.name}">
        	`;
        }
      ...
      <li  
	      ...
	      data-total-options="${each.options.length}" 
...
${button}
      ...
      let title;
      try {
        title = window.ezfyUpsellData.settings.appTitle;
      } catch (err) {
    ...
    <p class="EzfyCart-title">${title}</p>
  ...
  async function populatePopupWithProductData(productJSON) {
    ...
    /* Scroll popup to top */
    try {
      $popup.querySelector(`.EzfyPopup-container`).scrollTop = 0;
    } catch (err) {}
    ...
    for (var [index, each] of options.entries()) {
      ...
      if (each.type === "SELECT") {
        for (var [i, choice] of each.choices.entries()) {
          if (i === 0) {
            choicesHTML += `<option data-is-null selected>Please choose</option>`;
          }
        ...
        optionsHTML += `
			...
			class="EzfyProduct-option EzfyProduct-option--select" data-option-name="${
        each.name
      }">
				<span class="EzfyProduct-option-label">${each.name}</span>
				<select ${each.required ? "required" : ""}>
				${choicesHTML}
		</select>
			</div>
			`;
    ...
    <symbol id="checkmark" viewBox="0 0 24 24">
      ...
      if (each.type === "TEXTFIELD") {
        optionsHTML += `
			...
			class="EzfyProduct-option EzfyProduct-option--textfield" data-option-name="${
        each.name
      }">
				<span class="EzfyProduct-option-label">${each.name}</span>
				<input ${
          each.required ? "required" : ""
        } class="EzfyProduct-textfield-input" type="text" placeholder="Enter your text" />
			</div>
			`;
      }
      ...
      if (each.type === "DATE") {
        ...
        optionsHTML += `
			...
			class="EzfyProduct-option EzfyProduct-option--date" data-option-name="${
        each.name
      }">
				<span class="EzfyProduct-option-label">${each.name}</span>
				<input 
    ...
    $atc.addEventListener(`click`, async function (e) {
      ...
      const $error = document.querySelector(`.EzfyProduct-atc-errors`);
      ...
      for (var each of $options) {
        ...
        if (type === "select") {
          ...
          /* Error, empty */
          if (
            $select.options[$select.selectedIndex].hasAttribute("data-is-null")
          ) {
            const $field = $select.closest(`[data-option-index]`);
            errors.push($field);
          } else {
            value =
        ...
        if (type === "textfield") {
          ...
          if (
            ...
            const $field = $input.closest(`[data-option-index]`);
            errors.push($field);
          } else {
            value = _value;
          }
        ...
        if (type === "textarea") {
          ...
          var _value = $textarea.value.trim();

          if (
            $textarea.hasAttribute("required") &&
            (!_value || _value.length <= 0)
          ) {
            const $field = $textarea.closest(`[data-option-index]`);
            errors.push($field);
          } else {
            value = _value;
          }
        ...
        if (type === "date") {
          ...
          var _value = new Date($date.value)
            .getTime()
            .toString()
            .replace("NaN", "");

          if (
            $date.hasAttribute("required") &&
            (!_value || _value.length <= 0)
          ) {
            const $field = $date.closest(`[data-option-index]`);
            errors.push($field);
          } else {
            value = _value;
          }
      ...
      if (errors.length >= 1) {
        $error.textContent = `Please specify your options (${errors.length} left)`;
        return;
      } else {
        $error.textContent = ``;
      }
  ...
  async function makeSliderFixedHeight() {
    const $slider = await _waitForElement(`.EzfyCart-items.is-initialized`);
  ...
  async function addSliderToProducts() {
    ...
    await loadSplide();
    await _waitForElement(`.splide.EzfyCart-items`);
  ...
  async function start(restartType = null) {
    const products = await getUpsellProducts();

    await injectCartUpsell(products, restartType);
    injectPopupHTML();
    handleAddToCart();
    handlePopupClose();
    openProductLinksInPopup();
    handleInputOnChange();
    addSliderToProducts();
    makeSliderFixedHeight();

    // makeImagesSameHeight();
  }

  async function restart(restartType = null) {
    if (restartType === "cart_add") {
      var isRestart = true;
      start(restartType);
      return;
  ...
  function updatePriceOnChange($swatches) {
    const $price = document.querySelector(`[data-original-price]`);
    let price = parseFloat($price.getAttribute(`data-original-price`));

    const $options = document.querySelectorAll(
      `.EzfyProduct-option input:checked, .EzfyProduct-option select option:checked:not([data-is-null]):not([data-price="0"])`,
    );
    ...
    for (var each of $options) {
      price += parseFloat(each.getAttribute(`data-price`));
    }

    $price.textContent = window.Ecwid.formatCurrency(price);
  }
  ...
  return {
    ...
    init: async function () {
      ...
      window.Ecwid.OnCartChanged.add(async function (cart) {
        if (!window.firstCartRender) {
          window.firstCartRender = true;
          return;
        }
        restart((type = "cart_add"));
      });
